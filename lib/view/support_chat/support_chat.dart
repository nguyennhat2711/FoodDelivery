...
class _SupportChatState extends State<SupportChat> {
  ...
  getSupportChat() async {
    var response = await FirebaseFirestore.instance
        ...
        .get();
    if (response != null && response.docs.length > 0) {
      supportUserId = response.docs[0].data()['userId'].toString();
  ...
  sendStudentMessage(
      ...
      String message,
      String messageType,
      Function(bool isSend, String error) onMessageSent) async {
    try {
      if (chatList.length == 0) {
        Conversation conversation = Conversation(receiverId,
            FieldValue.serverTimestamp(), '', 'Merchant Support', null);
  ...
  sendMessage() async {
    ...
    if (message.isNotEmpty) {
      await sendStudentMessage(token, supportUserId, message, 'text',
          (isSuccess, error) {
    ...
    if (mounted)
      setState(() {
        if (pickedFile != null) {
          ...
          uploadMessageImage(_image, token, supportUserId, () {
            _scrollController.animateTo(
                _scrollController.position.maxScrollExtent,
                duration: const Duration(milliseconds: 500),
                curve: Curves.easeOut);
                  ...
                  GestureDetector(
                    onTap: () {
                      getCurrentLocation().then((value) {
                        ...
                        sendStudentMessage(token, supportUserId, location,
                            'location', (isSend, error) {});
                      });
                    },
                    child: Icon(
                      Icons.location_on,
                            ...
                            chatList[index].messageType == 'location'
                                ? Container(
                                    ...
                                    child: IgnorePointer(
                                      ...
                                      child: MapWidget(
                                        currentLocation: {
                                          "latitude": chatList[index]
                                              .message
                                              .split(',')[0],
                                          "longitude": chatList[index]
                                              .message
                                              .split(',')[1]
                                        },
                                      ),
  ...
  Future<http.Response> sendBlogNotification() {
    ...
    hashMap['include_external_user_ids'] = [supportUserId];
    return http.post(
