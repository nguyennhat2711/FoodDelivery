...
class OrderChatScreen extends StatefulWidget {
  ...
  OrderChatScreen(this.merchantId, this.merchantName, this.orderId);
...
class _OrderChatScreenState extends State<OrderChatScreen> {
    ...
    if (mounted)
      setState(() {
        if (pickedFile != null) {
          ...
          uploadMessageImage(_image, token, widget.merchantId, () {
    ...
    try {
      if (chatList.length == 0) {
        Conversation conversation = Conversation(receiverId,
            FieldValue.serverTimestamp(), '', widget.merchantName, null);
  ...
  sendMessage() async {
    ...
    if (message.isNotEmpty) {
      ...
      await sendStudentMessage(token, widget.merchantId, message, 'text',
          (isSuccess, error) {
                  ...
                  GestureDetector(
                    onTap: () {
                      getCurrentLocation().then((value) {
                        ...
                        sendStudentMessage(token, widget.merchantId, location,
  ...
  Future<http.Response> sendBlogNotification() {
    ...
    hashMap['include_external_user_ids'] = [widget.merchantId];
    return http.post(
